...
func (pr tcpProber) Probe(host string, port int, timeout time.Duration) (Result, error) {
...
func DoTCPProbe(addr string, timeout time.Duration) (Result, error) {
	...
	err = conn.Close()
	if err != nil {
		return Failure, fmt.Errorf("Unexpected error closing TCP probe socket: %v (%#v)", err, err)
	}
	return Success, nil
}
